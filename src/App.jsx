...
const STORAGE_KEY = "bottle-flip-v2";
...
function StatusBadge({ task }) {
  if (task.done) return <span style={{ fontSize: 11, background: "#e0e0e0", color: "#888", borderRadius: 10, padding: "2px 8px", fontWeight: 700 }}>完了✓</span>;
  if (task.endDate && isExpired(task.endDate)) return <span style={{ fontSize: 11, background: "#FF6B6B", color: "#fff", borderRadius: 10, padding: "2px 8px", fontWeight: 700 }}>期限切れ⚠</span>;
  if (task.endDate && isExpiringSoon(task.endDate)) return <span style={{ fontSize: 11, background: "#FF9F43", color: "#fff", borderRadius: 10, padding: "2px 8px", fontWeight: 700 }}>もうすぐ期限</span>;
  return null;
}
...
function PunishmentCard({ member, color, tasks, onAdd, onToggle, onDelete }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 20,
      overflow: "hidden",
      boxShadow: `0 4px 20px ${color.bg}40`,
      ...
      <div style={{ padding: "12px 14px", minHeight: 60 }}>
        {tasks.length === 0 && !adding && (
          <p style={{ color: "#bbb", textAlign: "center", fontSize: 13, margin: "10px 0" }}>
            まだ罰ゲームなし 🎉
          </p>
        )}
          ...
          return (
            <div key={task.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <button
                  onClick={() => onToggle(member, task.id)}
                  style={{
                    width: 26,
                    height: 26,
                    borderRadius: 8,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {(task.startDate || task.endDate) && (
                    <div style={{
                      fontSize: 12,
                      color: task.done ? "#bbb" : expired ? "#FF6B6B" : color.text,
                      fontWeight: 600,
                      marginTop: 3,
                    }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                  <button
                    onClick={() => onDelete(member, task.id)}
        ...
        {adding ? (
          <AddForm
            color={color}
            onAdd={(name, memo, startDate, endDate) => {
              onAdd(member, name, memo, startDate, endDate);
              setAdding(false);
            }}
          ...
          <button
            ...
            style={{
              ...
              background: "none",
              border: `1.5px dashed ${color.bg}88`,
              borderRadius: 10,
              padding: "8px",
              color: color.text,
              fontWeight: 700,
...
export default function App() {
  ...
  const loadFromStorage = useCallback(async (isBgSync = false) => {
    if (isBgSync) setSyncing(true);
    try {
    ...
    setLoading(false);
    if (isBgSync) setTimeout(() => setSyncing(false), 500);
  }, []);

  useEffect(() => {
    loadFromStorage(false);
    const interval = setInterval(() => loadFromStorage(true), POLL_INTERVAL);
  ...
  const handleAdd = (member, name, memo, startDate, endDate) => {
    save({
      ...data,
      [member]: [...(data[member] || []), { id: Date.now() + Math.random(), name, memo, startDate, endDate, done: false }],
    });
  };
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", padding: "32px 20px 20px" }}>
          ...
          <div style={{ marginTop: 8, fontSize: 11, color: syncing ? "#FFD700" : "rgba(255,255,255,0.3)", transition: "color 0.3s" }}>
            {syncing ? "🔄 同期中..." : lastSync ? `✓ ${lastSync.toLocaleTimeString("ja-JP")} 同期済み` : ""}
          </div>
        </div>
        ...
        <div style={{ textAlign: "center", marginTop: 30, color: "rgba(255,255,255,0.2)", fontSize: 12 }}>
          このアプリのリンクを共有すれば全員が同じデータをリアルタイムで見られます
        </div>
      </div>
    </>
  );
